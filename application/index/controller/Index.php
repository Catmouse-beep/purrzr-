    ...
    public function index() {
        ...
        try {
            ...
            $goods = $model_goods->with(['sku'])
                ->field('id, category_id, cover, name, is_sku')
                ->where("`category_id` in ({$category_ids})")
                ->where(['shelf' => 1])
                ->order('weigh desc')
                ->select();
        }catch (\Exception $e){
            echo '需要添加商品分类才可以访问';die;
        }

        $hot_goods = $model_goods->with(['sku'])
            ->field('id, category_id, cover, name, is_sku')
            ->where(['shelf' => 1])
            ->order('invented_sales desc, sales desc, weigh desc, id desc')
            ->limit(8)
            ->select();
        ...
        foreach($hot_goods as $key => $val){
            $hot_goods[$key] = $this->indexClGd($val, $agency);
        }
