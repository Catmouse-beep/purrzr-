...
//        echo $host;die;
        ...
        if(Session::has('template')){
            ...
            if(in_array($session_template, $template)){
                $this->template = $session_template;
            }else{
                $this->template = $template[mt_rand(0, count($template) - 1)];
        ...
        $this->template_config = file_exists(ROOT_PATH . 'plugin/' . $this->template . '/setting.php') ? include_once ROOT_PATH . 'plugin/' . $this->template . '/setting.php' : [];
